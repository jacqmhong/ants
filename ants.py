...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if not (self.place).ant or not ((self.place).ant).blocked:
            return False
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        destination = self.place.exit if self.direction == 'forward' else self.place.entrance
        # END EC
        if self.blocked():
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while not location.bees and location is not hive and count <= self.max_range:
            count += 1
            location = location.entrance
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        current_position = list((self.place).bees)
        if self.armor - amount <= 0:
            for bee in current_position:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        bee.reduce_armor(bee.armor)         # fix None-type bee from random_or_none
        # END Problem 6

    def action(self, colony):
        # BEGIN Problem 6
        if self.digesting > 0:              # if it is digesting,
            self.digesting -= 1             # decrement its digesting counter
        else:
            if (self.place).bees is not [] and random_or_none((self.place).bees):
                self.eat_bee(random_or_none((self.place).bees))     # otherwise eat a random Bee in its place by redcing the Bee's armor to 0
                self.digesting = self.time_to_digest                # and restart the digesting timer
        # END Problem 6
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        current_location = list((self.place).bees)
        for bee in current_location:
            bee.reduce_armor(self.damage)
        # END Problem 7
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if self.is_container is True and not self.contained_ant and not other.is_container:
            return True
        return False
        # END Problem 9
...
class ScubaThrower(ThrowerAnt):
    """More costly, watersafe, otherwise identical to its base class"""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.is_imposter = QueenAnt.exists
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        def double_ant_damage(stronger_ant):
            stronger_ant.damage = 2 * stronger_ant.damage
            self.enhancedAnts.append(stronger_ant)
        ...
        if self.is_imposter:
            ...
            while new_place:
                if new_place.ant:
                    if new_place.ant.is_container and new_place.ant.contained_ant and new_place.ant.contained_ant not in self.enhancedAnts:
                        double_ant_damage(new_place.ant.contained_ant)
                    elif new_place.ant not in self.enhancedAnts:
                        double_ant_damage(new_place.ant)
